...
export function getTemplateStyles(
  ...
  const margin =
    MARGIN_PRESETS[tid]?.[settings?.page_margin ?? "normal"] ??
    MARGIN_PRESETS[TEMPLATE_IDS.pragmatic].normal
...
```typescript
const { accent, background, font, density, margin, nameSize, uppercase, rightPanelColor } =
...
```typescript
const { accent, background, font, density, margin, nameSize, uppercase } = getTemplateStyles(
  resume.settings,
