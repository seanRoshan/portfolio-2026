 ...
 */
export async function resolvePrompt(slug: string, resumeId?: string): Promise<ResolvedPrompt> {
  ...
  return {
    ...
    user_prompt_template: override?.user_prompt_template ?? defaultPrompt.user_prompt_template,
...
export async function executePrompt(
  ...
  const userMessage = substituteVariables(prompt.user_prompt_template, variables)
 ...
 */
export async function listPrompts(category?: string): Promise<AIPrompt[]> {
  const supabase = await createClient()

  let query = supabase.from("ai_prompts").select("*").order("category").order("name")
