...
export function generateResumePdfHtml(resume: ResumeWithRelations): string {
  ...
  const baseDensity =
    DENSITY_MAP[resume.settings?.font_size_preset ?? "comfortable"] ?? DENSITY_MAP.comfortable
  ...
  const titleTransform =
    (sectionTitleUppercase ?? DEFAULT_UPPERCASE[templateId] ?? true) ? "uppercase" : "none"
  ...
  const sidebarTextColor =
    resume.settings?.sidebar_text_color ?? getContrastTextColor(backgroundColor)
  ...
  if (templateId === TEMPLATE_IDS.experienced) {
    return generateExperiencedHtml(
      resume,
      ci,
      df,
      accentColor,
...
/* ======================== PRAGMATIC ======================== */
function generatePragmaticHtml(
  resume: ResumeWithRelations,
  ci: ResumeWithRelations["contact_info"],
  df: DateFormat,
  accentColor: string,
  fontFamily: string,
  fontImportUrl: string,
  ...
  const contactLinks = [ci?.linkedin_url, ci?.github_url, ci?.portfolio_url]
    .filter(Boolean)
    .map((url) => {
  ...
  const contactParts: string[] = [
    ...
    ci?.city || ci?.state || ci?.country
      ? [ci?.city, ci?.state, ci?.country].filter(Boolean).join(", ")
      : null,
    ...contactLinks,
    ...
    .map((sid) => {
      switch (sid) {
        case "summary":
          ...
          return pragmaticSection(
            "SUMMARY",
            `<p style="line-height:${density.lineHeight};color:#374151;font-size:${density.body}">${escapeHtml(resume.summary.text)}</p>`,
            accentColor,
            density,
            titleTransform,
          )
        ...
        case "skills":
          ...
          return pragmaticSection(
            ...
            resume.skill_categories
              .map(
                (cat) =>
                  `<div style="margin-bottom:4px;font-size:${density.body}"><span style="font-weight:600">${escapeHtml(cat.name)}: </span><span style="color:#374151">${cat.skills.map(escapeHtml).join(", ")}</span></div>`,
              )
              .join(""),
            accentColor,
            density,
        ...
        case "certifications":
          ...
          return pragmaticSection(
            ...
            resume.certifications
              .map(
                (cert) =>
                  `<div class="experience-entry" style="display:flex;justify-content:space-between;margin-bottom:4px;font-size:${density.body}"><span><span style="font-weight:600">${escapeHtml(cert.name)}</span>${cert.issuer ? ` <span style="color:#6b7280">– ${escapeHtml(cert.issuer)}</span>` : ""}</span>${cert.date ? `<span style="color:#6b7280;font-size:${density.body}">${formatDate(cert.date, df)}</span>` : ""}</div>`,
              )
              .join(""),
            accentColor,
            density,
        ...
        case "extracurriculars":
          ...
          return pragmaticSection(
            ...
            resume.extracurriculars
              .map(
                (item) =>
                  `<div style="margin-bottom:4px;font-size:${density.body}"><span style="font-weight:600">${escapeHtml(item.title)}</span>${item.description ? ` <span style="color:#6b7280">– ${escapeHtml(item.description)}</span>` : ""}</div>`,
              )
              .join(""),
            accentColor,
            density,
    ...
    .map((sid) => {
      switch (sid) {
        case "summary":
          ...
          return monoSection(
            "SUMMARY",
            `<p style="line-height:${density.lineHeight};color:#000;margin:0;font-size:${density.body}">${escapeHtml(resume.summary.text)}</p>`,
            accentColor,
            density,
            titleTransform,
          )
        ...
        case "skills":
          ...
          return monoSection(
            ...
            resume.skill_categories
              .map(
                (cat) =>
                  `<div style="margin-bottom:3px;font-size:${density.body}"><strong>${escapeHtml(cat.name)}: </strong><span>${cat.skills.map(escapeHtml).join(", ")}</span></div>`,
              )
              .join(""),
            accentColor,
            density,
        ...
        case "certifications":
          ...
          return monoSection(
            ...
            resume.certifications
              .map(
                (cert) =>
                  `<div class="experience-entry" style="display:flex;justify-content:space-between;margin-bottom:3px;font-size:${density.body}"><span><strong>${escapeHtml(cert.name)}</strong>${cert.issuer ? ` <span style="color:#555">-- ${escapeHtml(cert.issuer)}</span>` : ""}</span>${cert.date ? `<span style="font-size:${density.body}">${formatDate(cert.date, df)}</span>` : ""}</div>`,
              )
              .join(""),
            accentColor,
            density,
        ...
        case "extracurriculars":
          ...
          return monoSection(
            ...
            resume.extracurriculars
              .map(
                (item) =>
                  `<div style="margin-bottom:3px;font-size:${density.body}"><strong>${escapeHtml(item.title)}</strong>${item.description ? ` <span style="color:#555">-- ${escapeHtml(item.description)}</span>` : ""}</div>`,
              )
              .join(""),
            accentColor,
            density,
...
/* ======================== SMARKDOWN ======================== */
function generateSmarkdownHtml(
  resume: ResumeWithRelations,
  ci: ResumeWithRelations["contact_info"],
  df: DateFormat,
  accentColor: string,
  ...
  const contactParts: string[] = [
    ci?.email
      ? `<a href="mailto:${escapeHtml(ci.email)}" style="color:${accentColor};text-decoration:none">${escapeHtml(ci.email)}</a>`
      : null,
    ci?.phone,
    ci?.city || ci?.state || ci?.country
      ? [ci?.city, ci?.state, ci?.country].filter(Boolean).join(", ")
      ...
      switch (sid) {
        case "summary":
          ...
          return smarkdownSection(
            "Summary",
            `<p style="line-height:${density.lineHeight};color:#333;margin:0;font-size:${density.body}">${escapeHtml(resume.summary.text)}</p>`,
            accentColor,
            density,
            titleTransform,
          )
        ...
        case "skills":
          ...
          return smarkdownSection(
            ...
            resume.skill_categories
              .map(
                (cat) =>
                  `<div style="margin-bottom:4px;font-size:${density.body}"><strong>${escapeHtml(cat.name)}: </strong><span style="color:#333">${cat.skills.map(escapeHtml).join(", ")}</span></div>`,
              )
              .join(""),
            accentColor,
            density,
        ...
        case "certifications":
          ...
          return smarkdownSection(
            ...
            resume.certifications
              .map(
                (cert) =>
                  `<div class="experience-entry" style="display:flex;justify-content:space-between;margin-bottom:4px;font-size:${density.body}"><span><strong>${escapeHtml(cert.name)}</strong>${cert.issuer ? ` <span style="color:#555">- ${escapeHtml(cert.issuer)}</span>` : ""}</span>${cert.date ? `<span style="color:#777;font-size:${density.body};font-style:italic">${formatDate(cert.date, df)}</span>` : ""}</div>`,
              )
              .join(""),
            accentColor,
            density,
        ...
        case "extracurriculars":
          ...
          return smarkdownSection(
            ...
            resume.extracurriculars
              .map(
                (item) =>
                  `<div style="margin-bottom:4px;font-size:${density.body}"><strong>${escapeHtml(item.title)}</strong>${item.description ? ` <span style="color:#555">- ${escapeHtml(item.description)}</span>` : ""}</div>`,
              )
              .join(""),
            accentColor,
            density,
...
/* ======================== CAREERCUP ======================== */
function generateCareerCupHtml(
  resume: ResumeWithRelations,
  ci: ResumeWithRelations["contact_info"],
  df: DateFormat,
  accentColor: string,
  ...
  const contactParts: string[] = [
    ...
    ...[ci?.linkedin_url, ci?.github_url, ci?.portfolio_url]
      .filter(Boolean)
      .map((url) => (url as string).replace(/https?:\/\/(www\.)?/, "").replace(/\/$/, "")),
  ].filter(Boolean) as string[]
  const visibleExps = resume.work_experiences.filter((e) => e.is_visible !== false)
      ...
      switch (sid) {
        case "summary":
          ...
          return careerCupSection(
            "SUMMARY",
            `<p style="line-height:${density.lineHeight};color:#222;margin:0;font-size:${density.body}">${escapeHtml(resume.summary.text)}</p>`,
            accentColor,
            density,
            titleTransform,
          )
        ...
        case "education":
          ...
          return careerCupSection(
            ...
            resume.education
              .map(
                (edu) =>
                  `<div class="experience-entry" style="margin-bottom:6px"><div style="display:flex;justify-content:space-between;align-items:baseline"><strong style="font-size:${density.heading}">${escapeHtml(edu.institution)}</strong>${edu.graduation_date ? `<span style="font-size:${density.body};color:#555;white-space:nowrap">${formatDate(edu.graduation_date, df)}</span>` : ""}</div><div style="font-size:${density.body};font-style:italic">${escapeHtml(edu.degree)}${edu.field_of_study ? ` in ${escapeHtml(edu.field_of_study)}` : ""}</div>${edu.gpa ? `<div style="font-size:${density.body};color:#555">GPA: ${edu.gpa}</div>` : ""}${edu.honors ? `<div style="font-size:${density.body};color:#555">${escapeHtml(edu.honors)}</div>` : ""}</div>`,
              )
              .join(""),
            accentColor,
            density,
        ...
        case "skills":
          ...
          return careerCupSection(
            ...
            resume.skill_categories
              .map(
                (cat) =>
                  `<div style="margin-bottom:2px;font-size:${density.body}"><strong>${escapeHtml(cat.name)}: </strong><span>${cat.skills.map(escapeHtml).join(", ")}</span></div>`,
              )
              .join(""),
            accentColor,
            density,
        ...
        case "certifications":
          ...
          return careerCupSection(
            ...
            resume.certifications
              .map(
                (cert) =>
                  `<div class="experience-entry" style="display:flex;justify-content:space-between;margin-bottom:2px;font-size:${density.body}"><span><strong>${escapeHtml(cert.name)}</strong>${cert.issuer ? `<span style="color:#555">, ${escapeHtml(cert.issuer)}</span>` : ""}</span>${cert.date ? `<span style="color:#555;font-size:${density.body}">${formatDate(cert.date, df)}</span>` : ""}</div>`,
              )
              .join(""),
            accentColor,
            density,
        ...
        case "extracurriculars":
          ...
          return careerCupSection(
            ...
            resume.extracurriculars
              .map(
                (item) =>
                  `<div style="margin-bottom:2px;font-size:${density.body}"><strong>${escapeHtml(item.title)}</strong>${item.description ? ` <span style="color:#555">- ${escapeHtml(item.description)}</span>` : ""}</div>`,
              )
              .join(""),
            accentColor,
            density,
...
function generateParkerHtml(
  ...
  sidebarColors: { primary: string; secondary: string; muted: string },
  margin: string,
): string {
  ...
  <div style="width:30%;background:${backgroundColor};color:${sidebarColors.primary};padding:${margin} 0.5in calc(${margin} - 0.2in);box-sizing:border-box;flex-shrink:0">
    ...
    <div style="margin-bottom:18px">
      ...
      <div style="font-size:${density.body};color:${sidebarColors.secondary};line-height:1.6">
        ...
        ${
          ci?.city || ci?.state || ci?.country
            ? `<div>${[ci?.city, ci?.state, ci?.country]
                .filter(Boolean)
                .map((s) => escapeHtml(s!))
                .join(", ")}</div>`
            : ""
...
<div style="background:#fff;min-height:11in">
  ...
  <div style="padding:${margin} 0.7in calc(${margin} - 0.2in);border-bottom:2px solid ${accentColor}">
    ...
    <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:6px;font-size:${density.body};color:#6b7280">
      ...
      ${
        ci?.city || ci?.state || ci?.country
          ? `<span>|  ${[ci?.city, ci?.state, ci?.country]
              .filter(Boolean)
              .map((s) => escapeHtml(s!))
              .join(", ")}</span>`
          : ""
