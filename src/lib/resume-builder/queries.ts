...
export async function getTemplates(): Promise<ResumeTemplate[]> {
  ...
  const { data } = await supabase.from("resume_templates").select("*").order("name")
  return (data as ResumeTemplate[]) ?? []
}

export async function getTemplate(id: string): Promise<ResumeTemplate | null> {
  const supabase = await createClient()
  const { data } = await supabase.from("resume_templates").select("*").eq("id", id).single()
...
export async function getResumeByShortId(shortId: string): Promise<Resume | null> {
  const supabase = await createClient()
  const { data } = await supabase
    .from("resumes")
    .select("*")
...
export async function getResumeContactInfo(resumeId: string): Promise<ResumeContactInfo | null> {
  const supabase = await createClient()
  const { data } = await supabase
...
export async function getResumeSummary(resumeId: string): Promise<ResumeSummary | null> {
  const supabase = await createClient()
  const { data } = await supabase
...
export async function getResumeWorkExperiences(resumeId: string): Promise<ResumeWorkExperience[]> {
  const supabase = await createClient()
  ...
  return experiences.map((exp) => ({
    ...exp,
    achievements:
      (achievements as ResumeAchievement[])?.filter((a) => a.parent_id === exp.id) ?? [],
  })) as ResumeWorkExperience[]
}

export async function getResumeEducation(resumeId: string): Promise<ResumeEducation[]> {
  const supabase = await createClient()
  const { data } = await supabase
...
export async function getResumeSkillCategories(resumeId: string): Promise<ResumeSkillCategory[]> {
  const supabase = await createClient()
  const { data } = await supabase
...
export async function getResumeProjects(resumeId: string): Promise<ResumeProject[]> {
  const supabase = await createClient()
  ...
  return projects.map((proj) => ({
    ...proj,
    achievements:
      (achievements as ResumeAchievement[])?.filter((a) => a.parent_id === proj.id) ?? [],
  })) as ResumeProject[]
}

export async function getResumeCertifications(resumeId: string): Promise<ResumeCertification[]> {
  const supabase = await createClient()
  const { data } = await supabase
...
export async function getResumeSettings(resumeId: string): Promise<ResumeSettings | null> {
  const supabase = await createClient()
  const { data } = await supabase
...
export async function getResumeWithRelations(
  ...
  // Fetch resume first to get template_id, then parallelize everything else
  ...
  const [
    ...
    resume.template_id ? getTemplate(resume.template_id) : null,
...
export async function getJobDescription(id: string): Promise<JobDescription | null> {
  const supabase = await createClient()
  const { data } = await supabase.from("job_descriptions").select("*").eq("id", id).single()
...
export async function getCoachSession(id: string): Promise<CareerCoachSession | null> {
  const supabase = await createClient()
  const { data } = await supabase.from("career_coach_sessions").select("*").eq("id", id).single()
