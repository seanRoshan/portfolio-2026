...
export function SmarkdownTemplate({ resume }: Props) {
  ...
  const { accent, font, density, margin, nameSize, uppercase } = getTemplateStyles(
    resume.settings,
    TEMPLATE_IDS.smarkdown,
  )
            ...
            <div key={exp.id} style={{ marginBottom: density.sectionGap }}>
              ...
              >
                <div>
                  ...
                  <span style={{ color: "#555", fontSize: density.heading }}>
                    {" "}
                    at {exp.company}
                  </span>
                </div>
        ...
        >
          {resume.education.map((edu) => (
            <div key={edu.id} style={{ marginBottom: "8px" }}>
              ...
              {edu.gpa && (
                <div style={{ fontSize: density.body, color: "#999" }}>GPA: {edu.gpa}</div>
              )}
              {edu.honors && (
                <div style={{ fontSize: density.body, color: "#999" }}>{edu.honors}</div>
              )}
            </div>
          ))}
        </Section>
        ...
        >
          {resume.projects.map((proj) => (
            <div key={proj.id} style={{ marginBottom: "8px" }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: "8px" }}>
                ...
                {proj.project_url && (
                  <a
                    ...
                    style={{ fontSize: "9.5px", color: accent, textDecoration: "none" }}
                  >
              ...
              {proj.description && (
                <div style={{ fontSize: density.body, color: "#333", marginTop: "2px" }}>
                  {proj.description}
                </div>
              )}
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: density.sectionGap }}>
        ...
        >
          {ci?.email && (
            <a href={`mailto:${ci.email}`} style={{ color: accent, textDecoration: "none" }}>
              {ci.email}
            </a>
          )}
