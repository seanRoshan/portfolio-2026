...
export function ExperiencedTemplate({ resume }: Props) {
  ...
  const { accent, background, font, density, margin, nameSize, uppercase, sidebarColors } =
    getTemplateStyles(resume.settings, TEMPLATE_IDS.experienced)
        ...
        >
          {resume.education.map((edu) => (
            <div key={edu.id} style={{ marginBottom: "8px" }}>
              <div
                style={{ fontWeight: 700, fontSize: density.heading, color: sidebarColors.primary }}
              >
                {edu.degree}
              </div>
              {edu.field_of_study && (
                <div style={{ fontSize: density.body, color: sidebarColors.secondary }}>
                  {edu.field_of_study}
                </div>
              )}
              <div style={{ fontSize: density.body, color: sidebarColors.secondary }}>
              ...
              {edu.graduation_date && (
                <div
                  style={{ fontSize: density.body, color: sidebarColors.muted, marginTop: "2px" }}
                >
        ...
        >
          {exps.map((exp) => (
            <div key={exp.id} style={{ marginBottom: density.sectionGap }}>
              ...
              >
                <div>
                  <span style={{ fontWeight: 700, fontSize: density.heading, color: "#111827" }}>
                    {exp.job_title}
                  </span>
                  <span style={{ color: "#6b7280", fontSize: density.heading }}>
                    {" "}
                    | {exp.company}
                  </span>
                </div>
        ...
        >
          {resume.projects.map((proj) => (
            <div key={proj.id} style={{ marginBottom: "8px" }}>
              ...
              {proj.description && (
                <div style={{ fontSize: density.body, color: "#374151", marginTop: "2px" }}>
                  {proj.description}
                </div>
              )}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {resume.target_role && (
          <div
            style={{ fontSize: density.section, color: accent, marginTop: "2px", fontWeight: 500 }}
          >
            {resume.target_role}
          </div>
        )}
        ...
        >
          ...
          {(ci?.city || ci?.state || ci?.country) && (
            <span>| {[ci?.city, ci?.state, ci?.country].filter(Boolean).join(", ")}</span>
          )}
          {links.map((link, i) => (
            <span key={i}>
              |{" "}
