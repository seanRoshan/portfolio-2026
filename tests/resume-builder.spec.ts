...
test.describe("Resume Builder", () => {
  ...
  test("editor page has all sections", async ({ page }) => {
    ...
    if (await firstLink.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      await expect(page.locator('[role="combobox"]').or(page.getByText(/template/i))).toBeVisible({
        timeout: 5000,
      })
      ...
      await expect(
        page
          ...
          .or(page.locator("button:has(svg.lucide-settings-2)")),
      ).toBeVisible()
    }
  })
  ...
  test("settings panel opens and has controls", async ({ page }) => {
    ...
    if (await firstLink.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const settingsBtn = page.locator("button:has(svg.lucide-settings-2)")
      if (await settingsBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        await expect(page.getByText("Section Visibility")).toBeVisible({ timeout: 3000 })
  ...
  test("score badge is visible", async ({ page }) => {
    ...
    if (await firstLink.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      await expect(page.locator("text=/[A-F]\\s*·\\s*\\d+/")).toBeVisible({ timeout: 5000 })
    }
  })
  ...
  test("template selector changes template", async ({ page }) => {
    ...
    if (await firstLink.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const templateSelect = page.locator('[role="combobox"]').first()
  ...
  test("drag handles are present for reordering", async ({ page }) => {
    ...
    if (await firstLink.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const gripHandles = page.locator('[aria-label="Drag to reorder"]')
  ...
  test("back button navigates to resume list", async ({ page }) => {
    ...
    if (await firstLink.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const backLink = page.locator('a[href="/admin/resume-builder"]')
      await expect(backLink).toBeVisible({ timeout: 3000 })
    }
  })

  test('"All changes saved" indicator is visible in editor', async ({ page }) => {
    await page.goto("/admin/resume-builder")
    const firstLink = page
      .locator('a[href*="/admin/resume-builder/"]')
      .filter({ has: page.locator("h3") })
